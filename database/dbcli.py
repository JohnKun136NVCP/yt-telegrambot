...
class usersdb:
    ...
    def close(self):
        ...
        self.connect.close()
class songsdb:
    def __init__(self,db_path="database/idSongs.db"):
...
def runcli():
    ...
    db_songs = songsdb()
    try:
        if sys.argv[1] == '--print-users' or sys.argv[1] == '--pu':
            db_users.print_all_users()
        elif sys.argv[1] == '--update-db' or sys.argv[1] == '--ud':
            db_users.updateDb()
        elif sys.argv[1] == '--change-user' or sys.argv[1] == '--cu':
            id_user = int(sys.argv[2])
            premium = bool(int(sys.argv[3]))
            type_user = sys.argv[4]
            db_users.changeUserType(id_user, premium, type_user)
        elif sys.argv[1] == '--force-song-reset' or sys.argv[1] == '--fsr':
            id_user = int(sys.argv[2])
            success, message = db_users.forceResetDailyCounts(id_user)
            print(success,message)
        elif sys.argv[1] == '--force-time-request' or sys.argv[1] == '--ftr':
            id_user = int(sys.argv[2])
            db_users.addRegisterTimeRequest(id_user)
        elif sys.argv[1] == '--force-time-request-all':
            db_users.registerAllRequestTimes()
        elif sys.argv[1] == '--print-songs' or sys.argv[1] == '--ps':
            db_songs.showidSongs()
        elif sys.argv[1] == '--delete-users-db' or sys.argv[1] == '--du':
            db_users.deleteDb()
        elif sys.argv[1] == '--delete-songs-db' or sys.argv[1] == '--ds':
            db_songs.deleteDb()
        elif sys.argv[1] == '--help' or sys.argv[1] == '--h':
            print("Usage:")
            print("  --print-users or --pu                 Print all users in the database.")
            print("  --update-db or --ud                   Update the database schema.")
            print("  --change-user or --cu <id> <premium> <type_user>  Change user type and premium status.")
            print("  --force-song-reset or --fsr <id>       Force reset daily song request count for a user.")
            print("  --force-time-request or --ftr <id>     Register current time as last request time for a user.")
            print("  --force-time-request-all or --fta      Register current time as last request time for all users.")
            print("  --print-songs or --ps            Print all songs in the songs database.")
            print("  --delete-users-db or --du        Delete all users from the database.")
            print("  --delete-songs-db or --ds        Delete all songs from the songs database.")
